  ...
  tagList(
    ...
    navbarPage(
      ...
      tabPanel("Run analysis",
               sidebarLayout(
               sidebarPanel(width = 3,style = "position:fixed;width:22%;border:1px solid #e0e0e0;border-radius:20px;", 
                            h5("Parameters"),
                            conditionalPanel(
                              'output.selectedFiles',
                            
                            numericInput("mzd2","Mass difference",value = 0.3),
                            checkboxGroupInput("runParameters", NULL, choiceNames = c("Export all plots (TIC, SIM, MS)", "Baseline correction", "Isotope correction", "correctTracerImpurity"),
